  ...
  #              phase = np.angle(mode_select)
   #             Z[phase < 0] = -Z[phase < 0]
    ...
    def plot_reconstructed_streamplot(self, u_ds_idx, v_ds_idx, mode_index):
        ...
        v_start_i, v_end_i = self.ds_idx_to_trainX_idx[v_ds_idx]  # Corrected variable name
        ...
        for snapshot in range(mode.shape[1]):
            ...
            fig, ax = plt.subplots(figsize=(8, 6)) 
            strm = ax.streamplot(x_grid, y_grid, u_interp, v_interp, density=[2,2]) #higher density = more lines
            ax.set_title(f"Mode: {mode_index}, {name}")
            ax.set_aspect("equal")
...
if __name__ == "__main__":
    ...
    # analysis.plot_streamplot_for_mode(ds_idx=0, scale=1)
    analysis.plot_reconstructed_streamplot(u_ds_idx=0, v_ds_idx=1, mode_index=15)

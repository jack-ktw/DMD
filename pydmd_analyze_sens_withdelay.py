...
class HankelDMDAnalysis(DMDAnalysisBase):
    ...
    def plot_timeseries(self, idx_li, snapshot):
        for idx in idx_li:
            ...
            plt.plot(pdata[idx, :], alpha=0.7, label=f"DMD Reconstructed Time Series", linewidth=2)
            plt.plot(self.train_X[:, idx], alpha=0.6, label="Original Time Series", linewidth=2)
            ...
            cumulative_errors = np.cumsum(np.sqrt(errors))
    ...
    def calc_average_error(self, idx_li, snapshot):
        ...
        for idx in idx_li:
            pdata = self.dmd.reconstructed_data
            error = (pdata[idx, :] - self.train_X[:, idx]) ** 2
            cumulative_error = np.cumsum(np.sqrt(error))
...
if __name__ == "__main__":
    ...
    num_snapshots_list = [300, 200, 100]
    delay_lengths = [1]
    average_errors_dict = {}
    ...
    for delay_length in delay_lengths:
        ...
        df.to_excel(os.path.join(save_dir, 'average_errors_200_300.xlsx'), index=False)
